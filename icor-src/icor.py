...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        choices=[
            ...
            'S3'],
        metavar="DATATYPE",
        help="Sentinel 2, Landsat-8 or Sentinel 3. Values : S2, L8 or S3",
        required=True)
    ...
    ap.add_argument(
        "--cloud_average_threshold",
        choices=[
            Range(
                0.0,
                1.0)],
        metavar="CLDAVGTHRSH",
        help="Upper threshold with average in the visual bands to be detected as cloud.",
        required=True)
    ap.add_argument(
        "--cloud_low_threshold",
        choices=[
            Range(
                ...
                1.0)],
        metavar="CLDLOWTHRSH",
        help="Low band threshold to be detected as cloud.",
        required=True)
    ap.add_argument(
        "--aot_window_size",
        type=int,
        metavar="AOTWINDOW",
        help="Square window size in pixels to perform aot estimation",
        required=True)
    ...
    ap.add_argument(
        ...
        choices=[
            Range(
                ...
                1.0)],
        metavar="WATERTHRSHD",
        help="Water detection threshold.",
        required=True)
    ...
    ap.add_argument(
        ...
        choices=[
            ...
            'false'],
        metavar="CIRRUS",
        help="Use cirrus band for cloud detection. Value : true or false",
        required=False)
    ap.add_argument(
        "--aot",
        choices=[
            ...
            'false'],
        metavar="AOT",
        help="Apply AOT retrieval algorithm. Value : true or false",
        required=True)
    ap.add_argument(
        "--simec",
        choices=[
            ...
            'false'],
        metavar="SIMEC",
        help="Apply adjacency correction. Value : true or false",
        required=True)
    ap.add_argument(
        "--watervapor",
        choices=[
            ...
            'false'],
        metavar="WATERVAPOR",
        help="Apply watervapor estimation. Value : true or false",
        required=False)
    ap.add_argument(
        "--bg_window",
        type=int,
        metavar="BG_WINDOW",
        help="Default background window size",
        required=True)
    ap.add_argument(
        "--cirrus_threshold",
        choices=[
            Range(
                0.0,
                1.0)],
        metavar="CIRRUSTHRESHOLD",
        help="Cloud mask threshold value",
        required=False)
    ap.add_argument(
        "--aot_override",
        choices=[
            Range(
                0.0,
                1.2)],
        metavar="AOT_OVERRIDE",
        help="AOT override values",
        required=True)
    ap.add_argument(
        "--ozone",
        choices=[
            ...
            'false'],
        metavar="OZONE",
        help="Use product ozone values. Value : true or false",
        required=False)
    ap.add_argument(
        "--ozone_override",
        choices=[
            Range(
                0.25,
                0.5)],
        metavar="OZONE_OVERRIDE",
        help="OZONE override values",
        required=True)
    ap.add_argument(
        "--wv_override",
        choices=[
            Range(
                ...
                5.0)],
        metavar="WV_OVERRIDE",
        help="WATERVAPOR override value",
        required=True)
    ...
    ap.add_argument(
        "--working_folder",
        metavar="WORKINGDIR",
        help="Location of intermediate date files",
        required=False)
    ...
    ap.add_argument(
        ...
        choices=[
            ...
            'false'],
        metavar="WORKINGDIR",
        help="Keep intermediate files. Value : true or false",
        required=True)
    ...
    ap.add_argument(
        "--sensor",
        metavar="SENSORNAME",
        help="Sentinel3 A or B sensor",
        required=False)
    ap.add_argument(
        "--apply_svc_gains",
        metavar="APPLY_SVC_GAINS",
        help="SVC gains apply or not",
        required=False)
    ap.add_argument(
        "--keep_land",
        metavar="KEEP_LAND",
        help="Keep all land pixels",
        required=False)
    ap.add_argument(
        "--keep_water",
        metavar="KEEP_WATER",
        help="Keep all water pixels",
        required=False)
