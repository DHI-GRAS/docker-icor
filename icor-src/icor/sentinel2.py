...
def sentinel2_granule(granule, thread_context, dir):
    ...
    if thread_context["aot"] == "true":
        ...
        if thread_context["watervapor"] == "true":
            ...
            aot_wv_string = "aot.override.watervapor=" + \
                thread_context["watervapor_override"] + "\n"
    ...
    if thread_context["simec"] == "true":
        ...
        simec_ozone_override_string = "simec.override.ozone=" + \
            thread_context["ozone_override"] + "\n"

        if thread_context["watervapor"] == "true" and returnCodeWV == 0:
            simec_wv_string = "simec.watervapor.location=" + water_vapor_60M + "\n"

        else:
            simec_wv_string = "simec.override.watervapor=" + \
                thread_context["watervapor_override"] + "\n"
    ...
    atm_watervapor_string = "atm.override.watervapor=" + \
        thread_context["watervapor_override"] + "\n"

    # construnct an output name
    output_dir = thread_context["output_file"]

    # =========================================================================
    thread_context.enter_stage("Atmospheric Correction - 10M")
    # =========================================================================

    result_10m = os.path.join(output_dir, "10M.tif")
    ...
    result_20m = os.path.join(output_dir, "20M.tif")
    # 20M
    if thread_context["watervapor"] == "true" and returnCodeWV == 0:
    ...
    # =========================================================================
    result_60m = os.path.join(output_dir, "60M.tif")
